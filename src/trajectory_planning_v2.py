        ...
        for i in range(len(x_interval)):
            for j in np.arange(y_interval[int(i)] + limit, y_interval[int(i)] + 2, 0.05):
                ...
                if generated_circle.can_be_generated(wall):
                    ...
                    # plt.plot(round(x_interval[i], 2), round(j, 2), 'ro')
                else:
                    continue
                id += 1
...
def greedy_cover(wall, C):
    ...
    while not wall.allcovered():
        ...
        # plot circle
        plt.scatter(max_circle.r_center[0], max_circle.r_center[1])
        # circle = plt.Circle((max_circle.r_center[0], max_circle.r_center[1]), max_circle.r_radius, fill=None, alpha=1)
